"""Round-Trip Translation (RTT) neutralization via a Mandarin pivot.

The neutralizer rewrites English prose through a Mandarin-then-back pass
(HSK3 vocabulary) to strip idiosyncratic author voice before style
transfer, matching the training-data generation process. Backends:
MLX (local, Qwen) and DeepSeek (API), selected via ``create_rtt_neutralizer``.
"""
